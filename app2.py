    ...
    if (hasParams) {
        ...
        if (!alreadySynced) {
            ...
            if (savedLat) {
                ...
                // Attempt automatic browser geolocation on first load
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        (error) => {
                            sessionStorage.setItem('tsn_synced', 'true');
                            // Fallback default coordinate if permission denied
...
def render_tsn_broadcast_center(lat, lon, loc_label):
  ...
  with col_radar:
    ...
    # Current telemetry metrics directly beneath radar
