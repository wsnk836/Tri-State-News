...
# --- NETWORK HEADER WITH EMBEDDED HERO BANNER / LOGO ---
st.markdown('<div class="tsn-header-bar"></div>', unsafe_allow_html=True)

# Attempt to load the local image file, with a clean HTML fallback if file context changes
try:
  st.image("777468448_1567274085055308_6458077729241826651_n.jpg", width=140)
except Exception:
  st.markdown(
      '<div style="background:#181922; border: 1px solid #ef4444; border-radius: 8px;'
      " padding: 10px; text-align: center; color: #ef4444; font-weight:"
      ' bold;">TSN NEWS NETWORK</div>',
      unsafe_allow_html=True,
  )
      ...
      if len(zip_input.strip()) == 5 and zip_input.strip().isdigit():
        try:
          ...
          if geo_resp:
            ...
            display_name = geo_resp[0].get(
                "display_name", f"ZIP {zip_input}"
            ).split(",")[0]
            loc_label = f"{
